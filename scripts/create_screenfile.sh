...
echo "screen ssh -o StrictHostKeyChecking=no -i anael.pem ubuntu@$bastion0 -t \"echo \"PS1=bastion0\"$\"\">>~/.bashrc;bash\" " >superscreen
echo "screen ssh -o StrictHostKeyChecking=no -i anael.pem ubuntu@$bastion1 -t \"echo \"PS1=bastion1\"$\"\">>~/.bashrc;bash\" " >> superscreen
echo "screen ssh -i anael.pem ubuntu@$frontend0 -o StrictHostKeyChecking=no -o ProxyCommand=\"ssh -o StrictHostKeyChecking=no -i anael.pem   -W %h:%p -q ubuntu@$bastion0\" -t \"echo \"PS1=frontend1\"$\"\">>~/.bashrc;bash\"" >>superscreen
