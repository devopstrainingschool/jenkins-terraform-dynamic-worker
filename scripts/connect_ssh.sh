...
["nfs0"]=`echo $inventory | jq '.nfs.hosts[0]'`) 
...
function connect_to_host  {
    ...
    then
        ...
        then
            screen ssh -o StrictHostKeyChecking=no -i /tmp/anael.pem ubuntu@$ssh_host -t "echo "PS1=$host_code"$"">>~/.bashrc;bash" 
        else
            screen ssh -i /tmp/anael.pem ubuntu@"$ssh_host" -o StrictHostKeyChecking=no -o ProxyCommand="ssh -o StrictHostKeyChecking=no -i /tmp/anael.pem   -W %h:%p -q ubuntu@"${cluster_hosts['bastion1']}"" -t "echo "PS1=$host_code"$"">>~/.bashrc;bash"
        fi
    else
        ...
        then
            ...
            new_window "ssh -o StrictHostKeyChecking=no -i /tmp/anael.pem ubuntu@$ssh_host -t \"echo \"PS1=$host_code\"$\"\">>~/.bashrc;bash\" "

        else

            new_window "ssh -i /tmp/anael.pem ubuntu@$ssh_host -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o ProxyCommand=\"ssh -o StrictHostKeyChecking=no -i /tmp/anael.pem   -W %h:%p -q ubuntu@${cluster_hosts['bastion1']}\" -t \"echo \"PS1=$\"$host_code\"\">>~/.bashrc;bash\""
